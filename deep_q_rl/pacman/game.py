...
class GameStateData:
  ...
  def __init__( self, prevState = None ):
    ...
    self.scoreChange = 0
    if prevState != None:
      self.scaredGhostEatenMultiplier = prevState.scaredGhostEatenMultiplier
      self.tick = prevState.tick
    else:
      self.scaredGhostEatenMultiplier = 1
      self.tick = 0
...
class Game:
  ...
  def run( self ):
    ...
    while not self.gameOver:
      ...
      # self.display.get_rgb()
      self.getStateMatrices(self.state)
  ...
  def getSelfStateMatrices(self):
    return self.getStateMatrices(self.state)
  ...
  def getStateMatrices(self, state):
      ...
      observation[1] = getFoodMatrix(state)
      observation[2] = getCapsulesMatrix(state)
      observation[3] = getPacmanMatrix(state)
      observation[4] = getGhostMatrix(state)
      observation[5] = getScaredGhostMatrix(state)

      observation = np.swapaxes(observation, 0, 2)
      if observation[:,:,3].sum() > 1:
        print("Multiple pacman!!", observation[:,:,3].sum())
