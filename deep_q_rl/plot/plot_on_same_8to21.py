...
def draw_plot(folder_name, dir, xlabel, color, legend_label):
    try:
...
sub_plot_ind = 1
i = 1
for size_prefix, folder_name in file_size_list:
    if prev_size_prefix != size_prefix:
        try:
            plt.subplot(4, 1, sub_plot_ind)
            plt.gca().set_xlim([0, 250])
            sub_plot_ind += 1
            i = 1
        except:
            print("Error saving ".format(folder_name))
    prev_size_prefix = size_prefix
    draw_plot(folder_name, os.path.join(root_folder, folder_name), "{}x{}".format(size_prefix, size_prefix), i, size_legend_labels[i])
    i += 1
...
fig.savefig("{}.jpg".format(os.path.join(root_folder, "maze_plot_by_size")))
plt.clf()
...
fig = plt.gcf()
fig.set_size_inches(18, 18)
fig.savefig("{}.jpg".format(os.path.join(root_folder, "maze_plot_by_type")))